...
import energy.controller.PedidoDeNovosContratosController;
...
public class TelaFuncionarioMenuPrincipal extends JFrame implements ActionListener {
    ...
    String[] colunas = {"ID", "Nome", "Data de Nascimento", "Distrito", "Contacto", "Email", "Localização"};
    ...
    public TelaFuncionarioMenuPrincipal(Usuario usuario){
    ...
    public void painelSuperiorDireito(){
        ...
        btnCadastrar.setBounds(600, 70, 120, 40);
        btnCadastrar.setBackground(colorPumpikin);

        btnAceitar.setBounds(600, 140, 120, 40);
        btnAceitar.setBackground(colorPumpikin);

        panelSuperiorDireito.add(btnCadastrar);
        panelSuperiorDireito.add(btnAceitar);
    ...
    public void preencherTodosDadosTabela(){
        model.setRowCount(0);
        PedidoDeNovosContratosController pedidoDeNovosContratosController = new PedidoDeNovosContratosController();

        //Ele tenta preencher a tabela com dados da base de dados
        try{
            ArrayList<PedidoDeNovosContratos> pedidoDeNovosContratos = pedidoDeNovosContratosController.listarTodosPedidos();
            pedidoDeNovosContratos.forEach((PedidoDeNovosContratos pedidoDeNovosContrato) -> {
                model.addRow(new Object[] {
                        pedidoDeNovosContrato.getId(),
                        pedidoDeNovosContrato.getNomeDoCliente(),
                        pedidoDeNovosContrato.getDistrito(),
                        pedidoDeNovosContrato.getContacto(),
                        pedidoDeNovosContrato.getContacto(),
                        pedidoDeNovosContrato.getEmail(),
                        pedidoDeNovosContrato.getLocalizacaoDaCasa()
                });
            });
    ...
    public void panelLateralEsquerdo(){
        ...
        btnContador.setBackground(colorPumpikin);
...
//
            ...
            try {
                PedidoDeNovosContratos pedidoDeNovosContratos = pedidoDeNovosContratosController.listarPedidoDeNovoContratoPorID(id);
                if(pedidoDeNovosContratos.isRespondido()){
                    JOptionPane.showMessageDialog(null, "Pedido já foi respondido");
                }else {
                    boolean isRespondido = pedidoDeNovosContratosController.responderPedidoDeNovoContrato(pedidoDeNovosContratos);
                    if (isRespondido) {
                        JOptionPane.showMessageDialog(null, "Pedido respondido com sucesso");

                    } else
                        JOptionPane.showMessageDialog(null, "Erro ao deletar funcionário");
                }
            }catch (ExceptionDAO ex){
                Logger.getLogger(TelaFuncionarioMenuPrincipal.class.getName()).log(Level.SEVERE, null, ex);
...
//            });
    ...
    public void actualizarContador(){
        ...
        PedidoDeNovosContratosController pedidoDeNovosContratos = new PedidoDeNovosContratosController();
        try {
            PedidoDeNovosContratos pedidoDeNovosContrato = pedidoDeNovosContratos.listarPedidoDeNovoContratoPorID(id);
            dispose();
            new TelaCadastrarCliente().preencherDadosNosCampos(pedidoDeNovosContrato, usuario);
        }catch (ExceptionDAO ex){
